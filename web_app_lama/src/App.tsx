...
import useImageSizeValidator from "@/hooks/useImageSizeValidator"
...
function Home() {
  ...
  const { processImageFile, RescaleDialog } = useImageSizeValidator()
    ...
    if (event.dataTransfer.files && event.dataTransfer.files.length > 0) {
      if (event.dataTransfer.files.length > 1) {
        ...
        if (SUPPORTED_FILE_TYPE.includes(fileType)) {
          processImageFile(dragFile).then((processed: File | null) => {
  ...
  const onPaste = useCallback((event: any) => {
    ...
    if (blob) {
      ...
      processImageFile(file).then((processed: File | null) => {
        if (processed) setFile(processed)
      })
    }
  }, [processImageFile, setFile])
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-start w-full bg-[radial-gradient(circle_at_1px_1px,_#8e8e8e8e_1px,_transparent_0)] [background-size:20px_20px] bg-repeat">
      <Toaster />
      <Header />
      <Workspace />
      {RescaleDialog}
