...
export function rescaleImageToShortSide(
  file: File,
  targetShortSide: number = MAX_IMAGE_DIMENSION
): Promise<File> {
  return new Promise((resolve, reject) => {
    const img = new Image()
    const url = URL.createObjectURL(file)
    img.onload = () => {
      ...
      canvas.toBlob(
        (blob) => {
          if (!blob) {
            reject(new Error("Error al reescalar la imagen"))
            return
          }
          const rescaledFile = new File([blob], file.name, {
            type: file.type,
